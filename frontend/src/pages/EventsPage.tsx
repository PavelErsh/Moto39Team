...
import { apiListRides, type RideItem } from '../api/rides'
    ...
    <section className="calendar-page">
      ...
      {loading ? (
        ...
        <div className="empty-state">
          ...
          <p className="muted">
            {filter === 'upcoming' && 'Пока нет предстоящих событий.'}
            {filter === 'past' && 'Прошедших событий нет.'}
            {filter === 'all' && 'Пока нет ни одного события.'}
          </p>
        </div>
      ) : (
        <ul className="events-list">
          {filtered.map(({ ev, date, lastDate }) => {
            ...
            return (
              ...
              >
                <Link to={`/rides/${ev.id}`} className="event-card__link">
                  {ev.cover_image_url && (
