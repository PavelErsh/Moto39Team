...
export default function HomePage() {
  ...
  // Кнопка «Я КАТАЮ»:
  //   1. СРАЗУ включаем статус ``riding`` на бэке — так плашка «🏍️ Я катаю
  //      (3 часа)» появляется над пейджером мгновенно, а на карте наша
  //      метка получает зелёную подпись «КАТАЮ». Статус сбрасывается сам
  //      через 3 часа (либо вручную кнопкой «Завершить поездку»).
  //   2. Одновременно показываем нативное уведомление о том, что режим
  //      активен (Web Notifications API — работает и в браузере, и в PWA).
  //   3. Только ПОСЛЕ этого открываем пост в Telegram. Если открытие
  //      делать первым (как у SOS/HELP), на мобильном браузере/Capacitor
  //      навигация переносит фокус на Telegram и pending fetch к API
  //      успевает быть отменён до отправки — статус на бэке не выставляется.
  //
  //   Заказчик отдельно просил, чтобы для «Я КАТАЮ» работало так же,
  //   как для SOS/HELP: сначала статус, потом всё остальное.
  const openYaKatayu = () => {
    // Показать уведомление можно только в ответ на пользовательский
    // жест — поэтому запускаем это сразу, не дожидаясь ответа сервера.
    ...
    ;(async () => {
      ...
      // После сохранения статуса открываем Telegram. `noopener` защищает
      // от tab-jacking. Fallback на same-tab срабатывает, если popup
      // заблокирован (некоторые режимы Safari / PWA standalone).
