...
import { apiGetRide, type RideItem } from '../api/rides'
...
export default function RideDetailPage() {
  const { id = '' } = useParams()
  const { user } = useAuth()
  const [item, setItem] = useState<RideItem | null>(null)
    ...
    return (
      <section className="references-page">
        <div className="empty-state">
          ...
          <p className="muted">{error ?? 'Событие не найдено'}</p>
  ...
  return (
    <article className="reference-detail">
      <div className="reference-detail__breadcrumbs">
        <Link to="/rides" className="reference-detail__back">
          ← События
        </Link>
        {user?.is_superuser && (
