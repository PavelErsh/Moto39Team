...
export interface User {
  ...
  created_at: string
  updated_at: string
  // Экстренный/активный статус пользователя.
  //   `null`      — обычный,
  //   `"help"`    — жёлтая подпись HELP на карте (сброс через 2 часа),
  //   `"sos"`     — красная SOS (сброс через 1 час),
  //   `"riding"`  — «я катаю»: зелёная подпись КАТАЮ (сброс через 3 часа).
  // Сброс выполняется на бэке; фронт лишь подтягивает актуальное значение
  // и раскрашивает собственный маркер / показывает плашку статуса.
  emergency_status?: 'help' | 'sos' | 'riding' | null
